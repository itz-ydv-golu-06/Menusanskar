...
function renderItemsTable() {
  ...
  tbody.innerHTML = items
    ...
    <tr data-id="${item.id}">
      <td>
        <div class="thumb">
          ${item.image ? `<img src="${item.image}" alt="${item.name}" style="width:100%;height:100%;object-fit:cover;">` : window.foodPlaceholder(item.name, item.category)}
        </div>
      </td>
...
/* Shared placeholder renderer (kept in sync with app.js / menu.js) */
window.foodPlaceholder = function (name, category) {
  const icons = {
